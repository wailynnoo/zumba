// admin-api/src/middleware/error.middleware.ts
// Centralized error handling middleware for Admin API
...
export function errorHandler(
  ...
  if (err && typeof err === "object" && "code" in err && err.code && typeof err.code === "string" && err.code.startsWith("P")) {
    ...
    res.status(statusCode).json({
      ...
      error: {
        ...
        meta: env.NODE_ENV === "development" ? prismaError.meta : undefined,
      },
    });
    return;
  }
  ...
  if (err && typeof err === "object" && err.constructor.name === "PrismaClientInitializationError") {
    res.status(503).json({
      success: false,
      message: "Database connection error. Please try again later.",
      error: env.NODE_ENV === "development" ? (err as Error).message : undefined,
    });
    return;
  }

  // Handle Prisma client unknown request errors
  if (err && typeof err === "object" && err.constructor.name === "PrismaClientUnknownRequestError") {
    res.status(500).json({
      success: false,
